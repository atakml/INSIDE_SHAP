...
for dataset in["ba2", "aids", "BBBP", "AlkaneCarbonyl", "Benzen", "mutag"]:#["mutag", "BBBP", "aids"]:#["AlkaneCarbonyl", "Benzen", "ba2"]:#["ba2", "Benzen"]: #["BBBP", "AlkaneCarbonyl", "aids"]:#["ba2", "aids", "BBBP", "AlkaneCarbonyl", "Benzen", "mutag"]:   
    cnt = 0 
    indices_to_ignore = []
    rule_dict = read_patterns(dataset)
    gnn_model = load_gnn(dataset)
    train_loader, graphs, features, labels, train_mask, val_mask, test_mask = load_dataset_gnn(dataset)
    ...
    for i, data in tqdm(enumerate(train_loader)):
        ...
        for value in rule_dict:
            node_mask = generate_node_values(feature, edge_index, gnn_model, [value]).bool()
