    ...
    for ii, pattern in rule_dict.items():
        ...
        for ell in range(layer + 1):
            ...
            if not nodes_to_add.shape[0]:
                break
            node_values[nodes_to_add] += value / (ell+1)
...
for dataset in ["BBBP", "AlkaneCarbonyl", "aids"]:#["ba2", "aids", "BBBP", "AlkaneCarbonyl", "Benzen", "mutag"]:   
    ...
    for i, data in tqdm(enumerate(train_loader)):
        ...
        for j in range(5, 50, 50):
            ...
            #edge_mask = calculate_edge_mask(data, node_values[i], j/100)
            edge_mask = calculate_edge_mask(data, edge_values[i], j/100, True)
            best_hfid = max(h_fidelity(data, gnn_model, edge_mask), best_hfid, key=lambda x: x[0])
        hfid += best_hfid
    print(f"{dataset=} metric: {hfid/len(train_loader)}")
        
        #with open(f"rule_masks_{dataset}_edge.pkl", "wb") as file:
        #    pickle.dump(edge_values, file)
