...
class Edgeshaper():
    ...
    def explain(self, M = 100, target_class = 0, P = None, deviation = None, log_odds = False, seed = None):
        ...
        for j in tqdm(range(num_edges)):
            ...
            for i in range(M):
                ...
                out = self.model(self.x, E_j_plus, batch=batch, edge_weights=edge_weight_j_plus)
                out_prob = None
                V_j_plus = None
                ...
                batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
                out = self.model(self.x, E_j_minus, batch=batch, edge_weights=edge_weight_j_minus)
    ...
    def explain_with_deviation(self, M = 100, target_class = 0, P = None, deviation = None, log_odds = False, seed = None, device = "cpu"):
        ...
        batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
        E = self.edge_index
        out = self.model(self.x, E, batch=batch, edge_weights=self.edge_weight)
    ...
    def compute_original_predicted_probability(self):
        self.model.eval()
        batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
        out_log_odds = self.model(self.x, self.edge_index, batch=batch, edge_weights=self.edge_weight)
