    ...
    if sample_method == "khop":
        ...
        if adj is None:
            adj = (
                ...
                .detach().to(data.x.device)
            )

        adj_self_loop = adj + torch.eye(data.num_nodes, device=data.x.device)
        ...
        sampled_nodes = torch.tensor(list(sampled_nodes), device = data.x.device)
