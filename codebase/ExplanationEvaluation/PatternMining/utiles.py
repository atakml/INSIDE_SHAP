...
def print_P(ds, P):
    ...
    print(" si ", truncate(sip), " ", truncate(P.SIP), " ", truncate(P.SIPUB), " || ", sim, " ", truncate(P.SIM), " ",
          truncate(P.SIMUB), " DL ", truncate(P.DLP), " ", truncate(P.DLPUB), " ", truncate(P.DLM), " ",
          truncate(P.DLMUB), " ratio", truncate(P.IR), P.signe)
...
def lecture_fichier(nom):
    ...
    nb_mol_plus = len(arome.loc[(slice(None), True), :])
    v_molecule = c[0]
...
def print_SG_file(ds, resultat):
    ...
    if (len(resultat) > 0):
        for k in range(len(resultat)):
            ...
            f_res.write("# " + str(k) + " target:" + str(cible) + " c+:" + str(P.nbClasse1) + " c-:" + str(
                P.nbMol - P.nbClasse1) + " score:" + str(P.IR) + " score+:" + str(P.SIP) + " score-:" + str(
                P.SIM) + " nb:" + str(len(P.P)) + " =")
            for i in P.P:
...
def build_model(nom_donnees, layer, cible=1):
    print("cible ", cible)
    global ds
    global Tclasse
    global Tmolecule
    ...
    for i in range(len(Tnames)):
        if f"l_{idLayer}" in Tnames[i]:
            layer_indices.append(i)
    indices = dict(enumerate(layer_indices))
    indices = {value: key for key, value in indices.items()}
    layer_indices = list(
        map(lambda x: indices[x], sorted(layer_indices, key=lambda x: int(Tnames[x][Tnames[x].find("c") + 2:]))))
    model.proba = model.proba[:, layer_indices]
    ds.data = ds.data[:, layer_indices]
    ds.pcol = ds.pcol[layer_indices]
    return model, ds, data, Tclasse, Tmolecule, Tatome, Tnames, pcol, pligne, nb_mol, nb_mol_plus


def compute_SI_double(probs, data, pattern, graph_inds, flag=False):
    ...
    if not len(negative_indices):
        return compute_SI_single(probs, data, pattern, graph_inds)
    DI = 0.6 * (len(positive_indices) + len(negative_indices)) + 1
    node_support_indices = np.where(~np.any(data[:, negative_indices], axis=1) & np.all(data[:, positive_indices], axis=1))
    ...
    graph_inds = graph_inds[node_support_indices]
    intervals = []
    start = 0
    current_ind = graph_inds[0]
    for i in range(probs.shape[0]):
        if current_ind != graph_inds[i]:
            intervals.append((start, i))
            start, current_ind = i, graph_inds[i]
...
def compute_SI_single(probs, data, pattern, graph_inds, flag=False):
    ...
    # graph_inds is the Tmolecule which indicates each row belongs to which graph
    ...
    if not len(node_support_indices[0]):
        return 0
    probs = probs[node_support_indices]
    data = data[node_support_indices]
    graph_inds = graph_inds[node_support_indices]
...
def read_from_mcts_files(file_name):
    f = open(file_name, 'r')
    lines = f.readlines()
    results = []
    regex = re.compile("Score: (.* )Pattern: (.* )Number of Simulations: (.*,) *")
    for line in lines:
        if "__" in line:
            continue
        else:
            groups = regex.findall(line)[0]
            results.append({'score': groups[0], 'pattern': groups[1], 'iterations': groups[2]})
    return results
...
def convert_subgroup_to_dict(subgroup):
    ...
    try:
        components = {comp.split("==")[0]: int(comp.split("==")[1].strip() == "True") for comp in components}
    except:
        return None
...
def read_from_beam_search_files(file_name):
    ...
    df["subgroup"] = df["subgroup"].apply(convert_subgroup_to_dict)
    df.rename(columns={"subgroup": "pattern", "quality": "score"}, inplace=True)
    return df[df.pattern.notnull()]


def get_support(pattern, data):
    positive_indices = list(filter(lambda index: pattern[index] == 1, pattern.keys()))
    node_support_indices = np.where(np.all(data[:, positive_indices], axis=1))
    return node_support_indices


def update_model(pattern, data, model):
    positive_indices = list(filter(lambda index: pattern[index] == 1, pattern.keys()))
    support = get_support(pattern, data)
    model.proba[support][positive_indices] = 0
    return model


def compare_mcts_with_beam_search(dataset_name, layer):
    mcts_file_name = f"mcts_{dataset_name}_{layer}"
    beam_search_file_name = f"beam_{dataset_name}_{layer}"
    mcts_data = read_from_mcts_files(mcts_file_name)
    beam_search_file_name = read_from_beam_search_files(beam_search_file_name)
...
def read_rules_from_file(file_name):
    ...
    layer_dict = {}
    for rule in rules:
        pattern_data = rule[rule.index("=") + 1:].split()
        pattern = {int(key[key.index('c') + 2:]): 1 for key in pattern_data}
        score = float(rule[rule.index("score:") + len("score:"): rule.index("score+") - 1])
        layer = int(pattern_data[0][pattern_data[0].index("_") + 1:pattern_data[0].index("c")])
        target = int(rule[rule.index(":") + 1])
        # print((layer, target))
        if (layer, target) not in layer_dict:
            layer_dict[(layer, target)] = [(pattern, score)]
        else:
            layer_dict[(layer, target)].append((pattern, score))
...
def read_log_probs_from_file(model_path):
    probs = []
    f = open(model_path, "r")
    lines = f.readlines()
    for line in lines:
        try:
            prob = list(map(float, line.strip("[ ]\n,").split(",")))
            probs.append(prob)
        except:
            print(line.strip("[ ]\n,").split(","))
    f.close()
