...
class InsideGNN(BaseExplainer):
    ...
    def __init__(self, model_to_explain, graphs, features, task, config,labels,**kwargs):
        ...
        self.config=config
        self.labels = labels
        self.rerun_extraction=True
        self.rerun_pattern_mining = True
        self.rerun_perturbation_serch = True
        self.policy_name = "node"
    ...
    def prepare(self, indices):
        ...
        filename = 'codebase/results/trace/'+self.config.dataset+"_"+self.config.model+"_"+self.config.explainer+"_activation.csv"
        ...
        try:
            ...
            filename = "codebase/results/trace/fidelity/"+self.config.dataset+"_"+self.config.model+"_"+self.config.explainer+"_"+pol_names+".csv"
    ...
    def _call_subgroup(self):
        ...
        code_dir = "codebase/ExplanationEvaluation/PatternMining/"
        cp_cmd= "cp codebase/results/trace/"+ file_name+".csv " +code_dir+cp_file_name+".csv"
        os.system(cp_cmd)

        for layer in range(4):#change for classics to 4 
        ...
        command4 = "(cd "+code_dir+ ";python  "+"posttraitement.py -i "+ cp_file_name+ ".csv -l " +str(4)+")"#change for classics to 4
        ...
        file = "codebase/ExplanationEvaluation/datasets/activations/"+ name+"/"+name+self.negation+"_encode_motifs.csv"
    ...
    def _embedding_to_df(self, labels =True):
        ...
        if isinstance(self.features,torch.Tensor) and len(self.features.shape) == 2:
        ...
        elif isinstance(self.features, list) or len(self.features.shape) == 3:
            embeddings = [list(map(lambda x : x.detach().cpu().numpy(),self.model_to_explain.embeddings(f,g))) for f, g in zip(self.features, self.graphs)]
            #embeddings = np.array(embeddings)#

            outputs = np.array([self.model_to_explain(f,g).detach().cpu().numpy() for f, g in zip(self.features, self.graphs)]).squeeze()
        else :
            raise NotImplementedError("Unknown graph data type")

        atoms = self.features
        #embeddings= np.array(embeddings)#
        if isinstance(self.features,torch.Tensor) and len(embeddings.shape) == 3 :
            embeddings= np.transpose(embeddings, [1,0,2])
            graph_sizes=1
        elif  isinstance(self.features, list) or len(embeddings.shape) == 4:
            layersize = embeddings[0][0].shape[-1]
            nlayer = len(embeddings[0])
            #embeddings = np.transpose(embeddings, [0,2,1,3])#
            embeddings = [np.hstack(embd) for embd in embeddings]#
            ...
            graph_sizes = [feature.shape[0] for feature in self.features]#
            #atoms = atoms.reshape((shape[0]*shape[1],-1))#
            atoms = torch.vstack(atoms)#
        ...
        d2 = [np.ones(j).reshape(-1, 1)*i for i,j in enumerate(graph_sizes)]#
        d2 = np.vstack(d2)#
        d2 = np.hstack((d2, embeddings))#
        ...
        inputs = pd.DataFrame(atoms.cpu().numpy(), columns=["l_3c_"+str(i) for i in range(len(atoms[0]))])#change for classics
        ...
        pred_bin = pd.DataFrame({"class" : [np.argmax(outputs[int(d2[el][0])])>0 for el in range(len(embeddings)) ]})#

        if labels:
            if len(self.labels.shape)==1:
                #pred_true = pd.DataFrame({"true_class" : [self.labels[el//graph_sizes].item()>0 for el in range(len(embeddings)) ]})#
                pred_true = pd.DataFrame({"true_class" : [self.labels[int(d2[el][0])].item()>0 for el in range(len(embeddings)) ]})#

            else :
                #pred_true = pd.DataFrame(
                #    {"true_class": [np.argmax(self.labels[el // graph_sizes]).item() > 0 for el in range(len(embeddings))]})#
                pred_true = pd.DataFrame(
                    {"true_class": [np.argmax(self.labels[int(d2[el][0])]).item() > 0 for el in range(len(embeddings))]})#
...
def labeled_rules(dataset,motif, negation):
    ...
    name = names[dataset]
    file = "ExplanationEvaluation/datasets/activations/"+ name+negation+"/"+name+negation+"_"+get_motifs_file(motif)+"_motifs.csv"
    #file = "ExplanationEvaluation/datasets/activations/"+ name+negation+"/"+name+negation+"_"+get_motifs_file(motif)+"_motifs_beam.csv"
    #file = "ExplanationEvaluation/datasets/activations/Mutag/Mutag_activation_double_beam.csv"
    #file = "ExplanationEvaluation/datasets/activations/"+ name+negation+"/"+name+negation+"_"+get_motifs_file(motif)+"_motifs_ex.csv"
    #file  = f"/home/ata/rules/{name}_activation.csv"
    print(file)
    rules = list()
