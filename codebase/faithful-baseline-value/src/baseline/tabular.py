...
def learn_baseline(args, model, X, edge_index, y, X_train):
    ...
    Args:
        ...
        X (torch 2d Tensor): sampled dataset. should be list object
        ...
        it is 2D, vstack of all feature matrices
    ...
    for itr in range(args.itr):
        ...
        loss, grads = calculate_grad(args, baselines, model, X, edge_index, y, low_orders)
...
def calculate_grad(args, baselines, model, X_input, edge_index, y_input, low_orders):
    ...
    for i in tqdm(range(num_of_features)):
        ...
        if args.vfunc == "log-odds":
            ...
            vS = compute_vS(model,input_batch, edge_index, one_hot_lbl, softmax_func, length, num_of_order, num_of_samples, num_of_inputs)
            vS_i = compute_vS(model,input_i_batch, edge_index, one_hot_lbl, softmax_func, length, num_of_order, num_of_samples, num_of_inputs)
